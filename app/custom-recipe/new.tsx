...
export default function NewCustomRecipeScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const { allIngredients, addCustomCocktail } = useApp();
  ...
  const toggleIngredientSelection = (id: string) => {
    const exists = selectedIngredients.find((s) => s.ingredientId === id);
    if (exists) {
      setSelectedIngredients(selectedIngredients.filter((s) => s.ingredientId !== id));
    } else {
      setSelectedIngredients([...selectedIngredients, { ingredientId: id, amount: '' }]);
    }
  };

  const updateIngredientAmount = (id: string, amount: string) => {
    setSelectedIngredients(selectedIngredients.map((s) => s.ingredientId === id ? { ...s, amount } : s));
  };
  ...
  const handleSave = () => {
    ...
    const id = `custom-cocktail-${Date.now()}`;
  ...
  return (
    <View style={styles.root}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <Text style={styles.title}>创建酒谱</Text>
        <Text style={styles.subtitle}>自定义你的专属配方</Text>
        ...
        <TouchableOpacity style={[styles.saveBtn, { marginBottom: insets.bottom + spacing.lg }]} onPress={handleSave} activeOpacity={0.7}>
          ...
          <Text style={styles.saveBtnText}>保存酒谱</Text>
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
